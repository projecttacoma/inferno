  ...
  module Sequence
    class USCore310DocumentreferenceSequence < SequenceBase
      ...
      def validate_resource_item(resource, property, value)
        ...
        when 'period'
          value_found = resolve_element_from_path(resource, 'context.period') do |period|
      ...
      test 'Server returns expected results from DocumentReference search by patient+status' do
        ...
        second_value = resolve_element_from_path(@documentreference_ary, 'status') { |el| get_value_for_search_param(el) != search_params[:status] }
